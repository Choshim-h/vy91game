...
<%
  flashname=trim(request("key"))
  if flashname=null  or len(flashname)<2  then
  	response.write "<script language=javascript>alert('关健字不能为空，且字符要大于一个');location.href='top.asp';'</script>"
  end if
%>
<html>
<head>
...
Sql="Select * From flash where flashname like'%"&flashname&"%' order by id desc"
...
%>
  <tr bgcolor="#f1f1f1">
    ...
    <td>&nbsp;&nbsp;<a href="../flash/<%=rs("id")%>.html" title=点击查看HTML页 target=_blank><%=rs("flashname")%></a></td>
    <td><div align="center">
...
Else
Response.Write "[<a href=""search.asp?Page=1&nclass="&request("nclass")&"&ClassID="& request("classID") &""">首 页</a>] "
Response.Write "[<a href=""search.asp?Page="& Page-1 &"&nclass="&request("nclass")&"&ClassID="& request("classID") &""">上一页</a>] "
End If
If Rs.PageCount-Page<1 Then
Response.Write "[下一页] [尾 页]"
Else
Response.Write "[<a href=""search.asp?Page="& Page+1 &"&nclass="&request("nclass")&"&ClassID="& request("classID") &""">下一页</a>] "
Response.Write "[<a href=""search.asp?Page="& Rs.PageCount &"&nclass="&request("nclass")&"&ClassID="& request("classID") &""">尾 页</a>]"
End If
