...
response.write"<b><font color=""#FF0000"">正在生成播放载入页,请等待......</font></b><BR><span id=showImport></span>"
   if request("p")<>"" then
     p=cint(request("p"))
     D=request("D")
   else
...
if allshu+www_888wang_com_sid<Sshu then'如果该生成的小于已生成的
 response.redirect "html_makeflashall_all_load.asp?type=ok&www_888wang_com_sid="&www_888wang_com_sid&"&www_888wang_com_eid="&www_888wang_com_eid&"&shu="&request("shu")&"&D="&d&"&p="&p+1&""
 response.end
else
 response.write"<meta http-equiv=""refresh"" content=""0;url='html_makeflashall_all_load.asp?type="&request("type")&"&www_888wang_com_sid="&www_888wang_com_sid&"&www_888wang_com_eid="&www_888wang_com_eid&"&shu="&Tshu&"&D="&D&"&p="&p+1&"'"">"
...
Response.Write "<iframe src='html_makeflash_load.asp?id="&id&"' width=90% height=35px></iframe>"

'生成到哪里
 Conn.Execute("update MakeConfig set PlayloadMake="&id)
...
response.write"<b>批量生成游戏播放载入页面"
if request("type")="ok" then
response.write"<br><br>操作成功:共生成<font color=""#FF0000"">"&request("www_888wang_com_eid")-request("www_888wang_com_sid")&"</font>页游戏播放载入页面<BR>总费时<font color=""#FF0000"">"&FormatNumber((timer()-request("D")),2)&"</font>秒,完成时间"&Now()&""
		'采集完成，转至生成分类页
end if
...
sql="select PlayLoadMake from MakeConfig"

rs.Open sql,conn,1,3
kaishiid=rs("PlayloadMake")
...
response.write"<form name=pid method=""post"" action=""html_makeflashall_all_load.asp?type=all"">超始ID<input size=8 name=""www_888wang_com_sid"" value='"&kaishiid&"'>到终止ID<input size=8 name=""www_888wang_com_eid"" value='"&jieshuid&"'><input name=""change"" class=""buttonface"" value=""确认"" type=""submit""></form>"
end if
