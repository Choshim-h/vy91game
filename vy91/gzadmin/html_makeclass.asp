...
Do While not Rs.Eof
i=i+1
top15=top15&"<li><span><a href='/"&makeUrl_class&"/"&Replace(classname," ","-")&"-1.html'>"&rs("classname")&"</a></span>"&i&"、<a href='/"&makeUrl_flash&"/"&Replace(rs("flashname")," ","-")&".html' target='_blank&play'>"&rs("flashname")&"</a></li>"
...
Do While not Rs.Eof
i=i+1
tj15=tj15&"<li><span><a href='#'>好评"&rs("love")&"</a></span><a href='/"&makeUrl_flash&"/"&Replace(rs("flashname")," ","-")&".html' target='_blank&play'>"&rs("flashname")&"</a></li>"
if i>=20 then exit do
rs.movenext
loop
rs.close
...
Sql="select top 8 id,flashname,nclass,classid,classname,tag,pic,hits from flash where nclass='"&requestClassID&"' order by hits desc"
Rs.open Sql,Conn,1,3
i=0
Do While not Rs.Eof
i=i+1
...
Rs.open("select id,flashname,nclass,classid,classname,spic,sj from flash where nclass='"&requestClassID&"' order by id Desc"),Conn,1,1
...
End If
 ...
 '==============常用标签start=================
	 ...
	 replaceString(3,0)="<!hotplay!>"
	 replaceString(3,1)=hotplay
