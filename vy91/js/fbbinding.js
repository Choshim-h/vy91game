...
function SendfacebookBinding(){
	...
	$.ajax({
		url:"/user/ajaxlogin.asp?action=ajax_SendfacebookBinding&reg_Email="+$reg_Email+"&reg_password="+$reg_password,
		type:'GET',
		success:function(){			
			if (arguments[0]=='pserror')
			{
				newaccount_t_message.style.display='block';
				$('#newaccount_t_message').html("Password is incorrect.");
			    return false;
			}else{
				newaccount_t_message.style.display='block';
