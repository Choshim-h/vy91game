...
top=top&"<li><a class='menu' href='/"&makeUrl_class&"/"&Replace(name," ","-")&"_1.html'>"&name&"</a></li>"
Rsadd.Movenext
Loop
Rsadd.Close
Set Rsadd=nothing
top=replace(top,"<a class='menu' href='/"&makeUrl_class&"/"&Replace(name," ","-")&"_1.html'>","<a class='menu2' href='/"&makeUrl_class&"/"&Replace(name," ","-")&"_1.html'>")
...
name=rs("name")
id2=rs("id")
top2=top2&"<li><a href='/"&makeUrl_class&"/"&Replace(name," ","-")&"_1.html' target='_blank'>"&name&"</a></li>"
rs.Movenext
Loop
rs.Close
...
'------------------------------------推荐小游戏图片
Sql="select top 14 * from flash where mpic<>'' and tag like '%html5%' order by newid()"
Rs.open Sql,Conn,1,3
Do While not Rs.Eof
tjxyx=tjxyx&"<div class=""thumb""><a href=""/gameinfo/"&Replace(Rs("flashname")," ","-")&".html""><img src="""&Rs("mpic")&""" alt="""&Rs("flashname")&"""></a></div>"
Rs.movenext
loop
Rs.close
...
if sqlConn=0 then
Sql="Select flash.id,flashname,cnflashname,flashurl,nclass,flash.classid,caozuo,flashwidth,flashheight,classname,pic,spic,size,sj,body,cnbody,tj,hits,hot,[user],zz,geshou,isclass,ysid,love,nolove,url,class.name From flash,class where Cint(nclass)=class.id and flash.ID="&requestClassID '查出一级分类名

else

Sql="Select flash.id,flashname,cnflashname,flashurl,nclass,flash.classid,caozuo,flashwidth,flashheight,classname,pic,spic,size,sj,body,cnbody,tj,hits,hot,[user],zz,geshou,isclass,ysid,love,nolove,url,class.name From flash,class where nclass=class.id and flash.ID="&requestClassID '查出一级分类名
end if
'response.Write(sql)
'response.End()
...
classLink="/"&makeUrl_class&"/"&Replace(nclassName," ","-")&"_1.html" '小类链接
nclassLink="/"&makeUrl_class&"/"&Replace(nclassName," ","-")&"_1.html" '大类链接
...
'------------------top15小游戏
Sql="select top 6 id,flashname,pic,classid,classname from flash where classid='"&classid&"' order by newid()"
Rs.open Sql,Conn,1,3
i=0
Do While not Rs.Eof
i=i+1
top15=top15&"<div class='flash_left_hot'><a href='/"&makeUrl_flash&"/"&Replace(rs("flashname")," ","-")&".html' title='"&rs("flashname")&"' target='_blank'><img src='/flashpic/"&rs("pic")&"'/></a></div>"
...
'------------------top16小游戏
Sql="select top 16 id,flashname,pic,classid,classname from flash order by newid()"
Rs.open Sql,Conn,1,3
i=0
Do While not Rs.Eof
i=i+1
top16=top16&"<div class='best_game_list'><a href='/"&makeUrl_flash&"/"&Replace(rs("flashname")," ","-")&".html' title='"&rs("flashname")&"' target='_blank'><img src='/flashpic/"&rs("pic")&"'/><br />"&left(rs("flashname"),17)&"</a></div>"
if i>=77 then exit do
rs.movenext
loop
rs.close
...
'==========页页标签开始==============
	 ...
	 replaceString2(4,1)=cnflashname
	 ...
	 replaceString2(29,0)="<!top16!>"
	 replaceString2(29,1)=top16
	 replaceString2(30,0)="<!spic!>"
	 replaceString2(30,1)=spic

	 
	 '==========页页标签结束==============
	 '888网(www.888wang.com)，专业娱乐网站程序，大量数据，自动更新，适合快速建站
	 mbUrl="gameplay/"&Replace(flashname," ","-")&".html"
