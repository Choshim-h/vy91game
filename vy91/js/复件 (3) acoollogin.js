    ...
    if(window.location.href.indexOf("?mobile")<0){
        try{
            if(/Android|webOS|iPhone|iPod|BlackBerry/i.test(navigator.userAgent)){
                window.location.href="http://m.acool.com/";
...
function Sendacoolsignup(){
	...
	$.ajax({
		url:"/user/ajaxlogin.asp?action=ajax_Sendacoolsignup&reg_Nickname="+$reg_Nickname+"&reg_Email="+$reg_Email+"&reg_Gender="+$reg_Gender+"&reg_password="+$reg_password+"&fbrz=0",
		type:'GET',
		success:function(){
...
function Send_forgot_password(){
	...
	$.ajax({
		url:"/user/forgot_password.asp?action=ajax_Send_forgot_password&reg_Email="+$reg_Email,
		type:'GET',
		success:function(){
			if (arguments[0]=='This Email has not been registered.')
			{
				newaccount_t_message.style.display='block';
				$('#newaccount_t_message').html(arguments[0]);
			}else{
				newaccount_t_message.style.display='block';
				newaccount_t_message.style.height='60px';
				$('#newaccount_t_message').html(arguments[0]);
				reg_Email.value="";
			}
...
function Send_get_password(){
	...
	if ($.trim($verify)=='')
		{
			newaccount_t_message.style.display='block';
			$('#newaccount_t_message').html("Data Error");
			return false;
		}
	...
	$.ajax({
		url:"/user/forgot_password.asp?action=ajax_Send_get_password&verify="+$verify+"&reg_Email="+$reg_Email+"&reg_password="+$reg_password,
		type:'GET',
		success:function(){
			if (arguments[0]=='Data Error')
			{
				newaccount_t_message.style.display='block';
				$('#newaccount_t_message').html(arguments[0]);
			}else{
  ...
  function facebook_API() {
    ...
    FB.api('/me', function(response) {
		...
		success:function(){
			if (arguments[0]=='pserror')
			{
				window.location.href="http://www.acool.com/user/fbregister.html"; 
